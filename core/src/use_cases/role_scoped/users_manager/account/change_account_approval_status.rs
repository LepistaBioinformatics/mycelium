...
on accounts with more privileges than himself.",
    ...
    if let UpdatingResponseKind::Updated(account) = &response {
        ...
        emit_resource_audit_event(
            ...
            serde_json::json!({ "action": "change_account_approval_status" }),
        )
        .await;
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn change_account_approval_status_emits_audit_event_on_success() {
    ...
    #[tokio::test]
    async fn change_account_approval_status_does_not_emit_audit_event_on_permission_error(
    ) {
