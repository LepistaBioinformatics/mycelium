...
| Postgres migrations | Raw SQL, operator-applied via `psql`. Base DDL `adapters/diesel_postgres/sql/up.sql`; incremental files in `adapters/diesel_postgres/sql/migrations/` named `YYYYMMDD_NN_<name>.sql` (at the time of writing, not folded back into `up.sql` — **superseded 2026-08-12** by `features/single-file-postgres-install/`: folding is now mandatory, see `CONVENTIONS.md` § "Postgres schema changes: two files, one commit"). `schema.rs` holds `diesel::table!` blocks. **No** `embed_migrations!` on the Postgres side (that exists only for SQLite/standalone). |
