use super::types::{ErrorResponse, UserLoginResponse, UserResponse};
...
pub async fn api_register_user(user_data: &str) -> Result<User, String> {
    let response =
        match http::Request::post("http://localhost:8000/api/auth/register")
...
pub async fn api_login_user(
    ...
    let response =
        match http::Request::post("http://localhost:8000/api/auth/login")
            .header("Content-Type", "application/json")
...
pub async fn api_user_info() -> Result<User, String> {
    let response =
        match http::Request::get("http://localhost:8000/api/users/me")
            .credentials(http::RequestCredentials::Include)
            .send()
            .await
        {
...
pub async fn api_logout_user() -> Result<(), String> {
    let response =
        match http::Request::get("http://localhost:8000/api/auth/logout")
            .credentials(http::RequestCredentials::Include)
            .send()
            .await
        {
