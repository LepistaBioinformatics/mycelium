          ...
          <div>
            ...
            if let Some(user) = user {
              <div class="flex items-center gap-8">
                <div class="mt-8">
                  <p class="mb-4">{format!("ID: {}", user.id.unwrap())}</p>
                  <p class="mb-4">{format!("Name: {}", user.first_name.unwrap_or("".to_string()).to_owned())}</p>
                  <p class="mb-4">{format!("Email: {}", user.email.get_email())}</p>
